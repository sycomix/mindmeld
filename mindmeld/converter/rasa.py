...
class RasaConverter(Converter):
    ...
    def _create_entities_directories(self, mindmeld_project_directory, entities):
        for entity in entities:
            entity_path = f"{mindmeld_project_directory}/entities/{entity}"
            self.create_directory(entity_path)
            with open(f"{entity_path}/gazetteer.txt", "w") as f:
                f.close()
            with open(f"{entity_path}/mapping.json", "w") as f:
    ...
    def _create_intent_training_file(self, intent_directory):
        ...
        with open(f"{intent_directory}/train.txt", "w") as f:
            f.close()
    ...
    def _translate_rasa_entry_to_mindmeld_entry(self, rasa_entry: str) -> str:
        ...
        for match, entity, entity_type in RASA_ENTITY_REGEX.findall(mindmeld_entry):
    ...
    def _add_example_to_training_file(self, current_intent_path: str, rasa_entry: str):
        with open(f"{current_intent_path}/train.txt", "a") as intent_f:
    ...
    def _get_action_endpoint(self):
        for file_ending in ["yaml", "yml"]:
            file_name = f"{self.rasa_project_directory}/endpoints.{file_ending}"
    ...
    def _read_domain_file(self):
        for file_ending in ["yaml", "yml"]:
            file_name = f"{self.rasa_project_directory}/domain.{file_ending}"
            if os.path.isfile(file_name):
                try:
                    with open(file_name, "r") as stream:
    ...
    def _read_entities(self):
        ...
        return domain_file["entities"] if "entities" in domain_file else []

    def _read_slots(self):
        domain_file = self._read_domain_file()
        return domain_file["slots"] if "slots" in domain_file else []
    ...
    def _get_stories(self):
        if os.path.isfile(f"{self.rasa_project_directory}/data/stories.md"):
            try:
                with open(f"{self.rasa_project_directory}/data/stories.md", "r") as f:
                    stories_dictionary = {}
                    current_story_name = ""
                    steps = []
                    current_step = {}
                ...
                logger.error(
                    ...
                    f"{self.rasa_project_directory}/data/stories.md",
                )
                logger.error(e)
        else:
            logger.error(
    ...
    def create_mindmeld_directory(self, mindmeld_project_path):
        ...
        self.create_directory(f"{mindmeld_project_path}/data")
        self.create_directory(f"{mindmeld_project_path}/domains")
        self.create_directory(f"{mindmeld_project_path}/domains/general")
        self.create_directory(f"{mindmeld_project_path}/entities")
    ...
    def create_mindmeld_training_data(self):
        ...
        if entities := [entity.lower() for entity in self._read_entities()]:
            self._create_entities_directories(self.mindmeld_project_directory, entities)

        # try and open data files from rasa project
        nlu_data_loc = f"{self.rasa_project_directory}/data/nlu_data.md"
        try:
        ...
        for line in nlu_data_lines:
            if self._is_line_intent_definiton(line):
                current_intent_path = f"{self.mindmeld_project_directory}/domains/general/{RasaConverter._get_intent_from_line(line)}"
                ...
                delimiter, rasa_entity = f'{line} '.split(' ', maxsplit=1)
        ...
        if len(entities) > 1:
            ...
            for entity_value in entities:
                ...
                if entity_value == entities[-1]:
                    entities_string += f"'{entity_string}']"
                else:
                    entities_string += f"'{entity_string}', "
        elif len(entities) == 1:
            for entity_value in entities:
                entity_string = entity_value.split(":")[0]
                entities_string += f"{has_entity_string}'{entity_string}'"
        return f"@app.handle(intent='{intent}'{entities_string}" + ")\n"

    def _write_function_declaration(self, action, f):
        if self._is_valid_function_name(action):
            function_declartion_string = f"def {action}(request, responder):\n"
    ...
    def _write_function_body_prompt(prompts, f):
        ...
        for prompt in prompts:
            ...
            if len(entities) > 0:
                ...
                prompts_list.append(f'"{newprompt}' + f'".format({entities_args})')
                for entity in entities_list:
                    newentity = entity.replace("{", "").replace("}", "")
                    entities_string = (
                        f"    {newentity}_s = [e['text'] for e in "
                ...
                prompts_list.append(f'"{prompt}"')
        prompts_string = f'    prompts = [{", ".join(prompts_list)}]\n'
    ...
    def _get_text_prompts_list(action_templates):
        return [
            template["text"] for template in action_templates if "text" in template
        ]
    ...
    def _read_file_lines(self):
        with open(f"{self.mindmeld_project_directory}/__init__.py", "r+") as f:
            return f.readlines()

    @staticmethod
    def _is_custom_action(action):
        return action[:6] == "action"
    ...
    def _write_functions(self, actions, templates, f):
        for action in actions:
            ...
            elif self._is_custom_action(action):
    ...
    def _attach_handle_to_function(handle, action, file_lines):
        for i, line in enumerate(file_lines):
            if f"def {action}(request, responder):" in line:
                insert_line = i
    ...
    def _attach_actions_to_function(current_action, actions, file_lines):
        ...
        current_line = next(
            (
                ...
                for i, line in enumerate(file_lines)
                if len(
                    re.findall("def {action}".format(action=current_action), line)
                )
                > 0
            ),
            None,
    ...
    def create_mindmeld_init(self):
        ...
        with open(f"{self.mindmeld_project_directory}/__init__.py", "w") as f:
            f.writelines(file_lines)

    @staticmethod
    def create_custom_features(mindmeld_project_directory, main_file_loc):
        with open(f"{main_file_loc}/rasa_custom_features.txt", "r") as f:
            string = f.read()
        with open(f"{mindmeld_project_directory}/custom_features.py", "w") as f:
            f.write(string)
