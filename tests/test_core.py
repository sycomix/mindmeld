...
def test_query_timestamp(query_factory):
    ...
    query = query_factory.create_query(
        "Today at noon", time_zone="America/Bahia", timestamp=1516748906000
    )
...
def test_sort_system_entities(query_factory):
    ...
    assert time_entities[18].entity.value["grain"] == "year"
    assert time_entities[17].entity.value["grain"] == "year"
    assert time_entities[16].entity.value["grain"] == "quarter"
    assert time_entities[15].entity.value["grain"] == "month"
    assert time_entities[14].entity.value["grain"] == "month"
    assert time_entities[13].entity.value["grain"] == "week"
    assert time_entities[12].entity.value["grain"] == "day"
    ...
    assert time_entities[5].entity.value["grain"] == "minute"
    assert time_entities[4].entity.value["grain"] == "minute"
    assert time_entities[3].entity.value["grain"] == "minute"
    assert time_entities[2].entity.value["grain"] == "minute"
    assert time_entities[1].entity.value["grain"] == "second"
    assert time_entities[0].entity.value["grain"] == "second"
